...
import actionsMapAssets from '../assets/assets-actions';
...
): (diff: Delta, newObj: Category, oldObj: Category) => Array<UpdateAction> {
  return function doMapActions(
    diff: Delta,
    newObj: Category,
    oldObj: Category /* , options */
  ): Array<UpdateAction> {
    const allActions = [];
    ...
    allActions.push(
      mapActionGroup(
        ...
        (): Array<UpdateAction> =>
          actionsMapAssets(
            diff,
            oldObj as unknown as Asset,
            newObj as unknown as Asset
          )
      )
    );
...
export default (
  ...
  const buildActions = createBuildActions(
    diffpatcher.diff,
    doMapActions as () => CategoryUpdateAction[],
    copyEmptyArrayProps
  );
