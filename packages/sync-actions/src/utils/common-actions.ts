...
export const createIsEmptyValue =
  <T extends object | string>(emptyValues: Array<T>) =>
  (value: string | unknown) =>
    emptyValues.some((emptyValue) => emptyValue === normalizeValue(value));

/* actions that start with 'set' can generate undefined valued */
export const isOptionalField = (action: string) => action.startsWith('set');
 ...
 */
export function buildBaseAttributesActions<T, U>({
  actions,
  diff,
  oldObj,
  newObj,
  ...
  return actions
    .map((item) => {
      ...
      if (
        isNotDefinedNow &&
        !isOptionalField(item.action as string) &&
        shouldPreventUnsettingRequiredFields
      )
        return undefined;
 ...
 * @param  {Object} options.newObj - the new representation of the object
 */
export function buildReferenceActions<T>({
  actions,
  diff,
