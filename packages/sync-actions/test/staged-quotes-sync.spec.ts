...
describe('Actions', () => {
  let stagedQuotesSync: SyncAction<Quote, QuoteUpdateAction>;
  beforeEach(() => {
    stagedQuotesSync = createStagedQuotesSync([], {});
  });

  test('should build `changeQuoteState` action', () => {
    const before = { stagedQuoteState: 'InProgress' } as Partial<Quote>;
    const now = { stagedQuoteState: 'Sent' } as Partial<Quote>;
