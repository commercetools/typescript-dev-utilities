import productDiscountsSyncFn, {
  actionGroups,
} from '../src/product-discounts/product-discounts';
import { baseActionsList } from '../src/product-discounts/product-discounts-actions';
import {
  ProductDiscount,
...
describe('Actions', () => {
  let productDiscountsSync: SyncAction<
    ProductDiscount,
    ProductDiscountUpdateAction
  >;
  ...
  test('should build the `changeValue` action', () => {
    ...
    const actual = productDiscountsSync.buildActions(
      now as Partial<ProductDiscount>,
      before as Partial<ProductDiscount>
    );
    expect(actual).toEqual(expected);
  });
