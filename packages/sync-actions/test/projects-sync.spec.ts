import createProjectsSync, { actionGroups } from '../src/projects/projects';
import {
  baseActionsList,
...
describe('Actions', () => {
  let projectsSync: SyncAction<Project, ProjectUpdateAction>;
  beforeEach(() => {
    ...
    describe('given `shippingRateInputType` is of type `CartClassification`', () => {
      ...
      describe('given a value of `values` changes', () => {
        ...
        test('should build `setShippingRateInputType` action', () => {
          const actual = projectsSync.buildActions(
            now as Partial<Project>,
            before as Partial<Project>
          );
          const expected = [
            {
              action: 'setShippingRateInputType',
  ...
  test('should build `changeMessagesConfiguration` action', () => {
    const before = {
      messagesConfiguration: { type: 'some-config' },
    } as Partial<Project>;
    const now = {
      messagesConfiguration: { type: 'some-other-config' },
    } as Partial<Project>;
    const actual = projectsSync.buildActions(now, before);
    const expected = [
      {
