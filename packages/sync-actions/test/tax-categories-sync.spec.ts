import taxCategorySyncFn, {
  actionGroups,
} from '../src/tax-categories/tax-categories';
import { baseActionsList } from '../src/tax-categories/tax-categories-actions';
import {
  SyncAction,
  TaxCategory,
...
describe('Actions', () => {
  let taxCategorySync: SyncAction<TaxCategory, TaxCategoryUpdateAction>;
  beforeEach(() => {
  ...
  test('should build `addTaxRate` action', () => {
    const before = { addresses: [] } as Partial<TaxCategory>;
    const now = {
      rates: [
        {
          name: '5% US',
  ...
  test('should build `replaceTaxRate` action', () => {
    ...
    } as unknown as Partial<TaxCategory>;
